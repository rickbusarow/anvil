...
public abstract class AnvilExtension @Inject constructor(
   ...
   * this module. That often means you can skip KAPT and the stub generating task. In addition,
   ...
   * then also [generateDaggerFactories] must be set to `true`.
   *
   * This property can also be set via a Gradle property:
   *
   * ```properties
  ...
  /**
   * Add the `annotations-optional` artifact as a dependency to make annotations like `@SingleIn`
   * and `@ForScope` available to use. These are annotations that are not strictly required but
   * which we've found to be helpful with managing larger dependency graphs.
   *
   * This property can also be set via a Gradle property:
   *
   * ```properties
  ...
  /**
   * Configures each variant of this project. For Android projects these are the respective Android
   * variants, for JVM projects these are usually the main and test variant.
   */
